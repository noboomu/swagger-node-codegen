...
const swagger2 = async swagger => {
  ...
  const pKeys= Object.keys(swagger.paths);
  ...
  _.each(swagger.paths, (path, path_name) => {
    ...
    _.each(path, (method, method_name) => {
      ...
      method.operationId = (method.operationId || generateOperationId(method_name, path_name).replace(/\s/g, '-'));
      
      const extensionKeys = Object.keys(method).filter( p => p.indexOf('x-') > -1);

      if(extensionKeys.length > 0)
      {
        method.extensions = {};

        extensionKeys.forEach( k => {

            method.extensions[k] = method[k];
